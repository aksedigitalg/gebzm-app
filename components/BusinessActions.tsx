...
export function BusinessActions({ businessName, businessType, bookingLabel = "Rezervasyon", services, businessId }: Props) {
  ...
  return (
    <>
      {/* CTA Bar — viewport altından 20px, sidebar hariç ortalı */}
...
function BookingDialog({ open, onClose, businessName, bookingLabel, businessId }: {
  open: boolean; onClose: () => void; businessName: string;
  bookingLabel: string; businessId?: string;
}) {
  ...
  const submit = async () => {
    ...
    setError("");
    const user = getUser();
    if (!user?.token || !businessId) { setError("Giriş yapmanız gerekiyor."); setLoading(false); return; }
    try {
      ...
      await api.user.startConversation(businessId,
        `${bookingLabel} talebim: ${new Date(date).toLocaleDateString("tr-TR")} saat ${time}`
      );
      setConfirmed(true);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} title={`${businessName} · ${bookingLabel}`}>
      {confirmed ? (
        <div className="flex flex-col items-center py-8 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500">
            <Check className="h-8 w-8" strokeWidth={2} />
          </div>
          <p className="mt-4 text-base font-bold">{bookingLabel} Talebiniz Alındı</p>
          <p className="mt-1 max-w-xs text-xs text-muted-foreground">
            Mesajlarım bölümünden işletme yanıtını takip edebilirsiniz.
          </p>
...
function QuestionDialog({ open, onClose, businessName, businessType, businessId }: {
  ...
  return (
    <Dialog open={open} onClose={handleClose} title={`${businessName} · Soru Sor`}>
      {sent ? (
        <div className="flex flex-col items-center py-8 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-500">
            <Check className="h-8 w-8" strokeWidth={2} />
          </div>
          <p className="mt-4 text-base font-bold">Mesajınız Gönderildi</p>
          <p className="mt-1 max-w-xs text-xs text-muted-foreground">
            İşletme yanıtı Mesajlarım bölümüne düşecek.
          </p>
          <button type="button" onClick={handleClose}
            className="mt-5 inline-flex items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground transition hover:opacity-90">
            Tamam
