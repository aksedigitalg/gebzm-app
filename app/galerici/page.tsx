...
async function getBusinesses(): Promise<Biz[]> {
  try {
    ...
    return all.filter((b: { type: string }) => b.type === "galerici");
  } catch { return []; }
}
...
export default async function Page() {
  ...
  return (
    <>
      <PageHeader title="Oto Galeriler" subtitle="Sıfır & ikinci el araçlar" back="/kategoriler" />
      <div className="px-5 pb-6 pt-4">
        ...
        {businesses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <Car className="h-12 w-12 text-muted-foreground/40" strokeWidth={1.5} />
            <p className="mt-4 text-sm font-semibold">Henüz oto galeri eklenmemiş</p>
            <p className="mt-1 text-xs text-muted-foreground">Galeriler platforma katıldıkça burada listelenecek.</p>
          </div>
        ) : (
          <div className="space-y-3 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
            {businesses.map((b) => (
              <Link key={b.id} href={`/hizmetler/${b.id}`}
                ...
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-600 to-zinc-700 text-white overflow-hidden">
                  {b.logo_url
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="shrink-0 rounded-full bg-gradient-to-br from-slate-600 to-zinc-700 px-2 py-0.5 text-[10px] font-bold text-white">Galerici</span>
                  </div>
