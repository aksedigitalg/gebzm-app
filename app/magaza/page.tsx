...
async function getBusinesses(): Promise<Biz[]> {
  try {
    ...
    return all.filter((b: { type: string }) => b.type === "magaza");
  } catch { return []; }
}
...
export default async function Page() {
  ...
  return (
    <>
      <PageHeader title="Mağaza" subtitle="Giyim, elektronik, ev & yaşam" back="/kategoriler" />
          ...
          <div className="space-y-3 lg:grid lg:grid-cols-2 lg:gap-4 lg:space-y-0">
            {businesses.map((b) => (
              <Link key={b.id} href={`/hizmetler/${b.id}`}
                ...
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-500 to-blue-600 text-white overflow-hidden">
                  {b.logo_url
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="shrink-0 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 px-2 py-0.5 text-[10px] font-bold text-white">Mağaza</span>
                  </div>
