...
const categories = [
  ...
  { id: "elektronik", label: "Elektronik", subs: ["Telefon", "Laptop", "Tablet", "Televizyon", "Beyaz Eşya", "Diğer"] },
  { id: "ev-yasam", label: "Ev & Yaşam", subs: ["Mobilya", "Koltuk Takımı", "Mutfak", "Bahçe", "Diğer"] },
...
export default function YeniIlanPage() {
  ...
  // Edit modunda mevcut ilanı yükle
  useEffect(() => {
    if (!editId) return;
    api.user.getMyListings().then((data) => {
      ...
      if (item) {
        ...
        setSubCategory((item.sub_category as string) || "");
        setPrice(String(item.price || ""));
        setLocation((item.location as string) || "");
        setPhotos((item.photos as string[]) || []);
      }
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!title || !category || !price) { setError("Başlık, kategori ve fiyat zorunludur."); return; }
    setLoading(true);
    try {
      const featureMap: Record<string, string> = {};
      features.filter((f) => f.key.trim()).forEach((f) => { featureMap[f.key] = f.value; });
      const payload = {
        title, category, sub_category: subCategory,
        price: parseInt(price.replace(/\D/g, "")) || 0,
        description, location, features: featureMap, photos,
      };
      if (isEdit) {
        await api.user.updateListing(editId, payload);
      } else {
        await api.user.createListing(payload);
      }
      setStep("done");
    } catch (err) {
    ...
    return (
      <div className="flex min-h-[calc(100dvh-76px)] flex-col items-center justify-center px-5 text-center">
        ...
        <h2 className="mt-5 text-xl font-bold">
          {isEdit ? "İlan Güncellendi!" : "İlan Yayında!"}
        </h2>
        <p className="mt-2 max-w-xs text-sm text-muted-foreground">
          İlanınız başarıyla {isEdit ? "güncellendi" : "oluşturuldu"}.
        </p>
        <div className="mt-6 flex gap-3">
          <Link href="/ilanlar" className="rounded-full border border-border bg-card px-5 py-2.5 text-sm font-semibold transition hover:bg-muted">
            İlanlara Git
          </Link>
          <Link href="/profil/ilanlarim" className="rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground transition hover:opacity-90">
            İlanlarım
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-[calc(100dvh-76px)]">
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-card/95 px-5 py-3 backdrop-blur">
        <Link href="/profil/ilanlarim" className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <h1 className="text-base font-semibold">{isEdit ? "İlanı Düzenle" : "Yeni İlan Ver"}</h1>
      </div>
      ...
      <form onSubmit={submit} className="space-y-5 px-5 pb-8 pt-5">
        ...
        <section className="space-y-4 rounded-2xl border border-border bg-card p-4">
          ...
          <div>
            ...
            <textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="İlanınız hakkında detaylı bilgi..."
        ...
        <section className="space-y-3 rounded-2xl border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={addFeature} className="flex items-center gap-1 text-xs font-medium text-primary hover:underline">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? (isEdit ? "Güncelleniyor..." : "Yayınlanıyor...") : (isEdit ? "Güncelle" : "İlanı Yayınla")}
        </button>
      </form>
    </div>
  );
